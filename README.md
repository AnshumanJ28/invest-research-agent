...
End-to-end smoke test run against `RELIANCE.NS`, executing the native C++ pipeline from the command line:

```powershell
.\build\Release\invest_pipeline.exe RELIANCE.NS
```

*(Note: The visual screenshots from the deprecated Python pipeline have been removed. The current C++ output streams directly into the terminal and seamlessly populates the `invest.sqlite` database).*
...
```
*(Note: To persist the `invest.sqlite` database to your host machine, mount the directory: `-v ${PWD}/cpp_pipeline:/app/cpp_pipeline`)*
