...
std::vector<FinancialStatement> parse_yfinance_json(const json& root_json) {
    ...
    if (result.contains("incomeStatementHistory") && result["incomeStatementHistory"].contains("incomeStatementHistory")) {
    ...
    if (result.contains("balanceSheetHistory") && result["balanceSheetHistory"].contains("balanceSheetStatements")) {
    ...
    if (result.contains("cashflowStatementHistory") && result["cashflowStatementHistory"].contains("cashflowStatements")) {
    ...
    if (result.contains("defaultKeyStatistics")) {
        auto ks = result["defaultKeyStatistics"];
        ...
        for (const auto& pair : cmap) {
            ...
            if (ks.contains(yf_key) && ks[yf_key].is_object() && ks[yf_key].contains("raw")) {
                key_stats.line_items.push_back({canonical, ks[yf_key]["raw"].get<double>(), "mixed"});
            }
        }
    }
...
std::map<std::string, EdgarDocument> parse_edgar_json(const json& j) {
    std::map<std::string, EdgarDocument> docs;
    if (!j.is_object()) return docs;
    for (auto& el : j.items()) {
...
TranscriptDocument parse_transcript_json(const json& j) {
    TranscriptDocument doc;
    doc.available = false;
    if (!j.is_object()) return doc;
    doc.available = j.value("available", false);
    doc.unavailable_reason = (j.contains("unavailable_reason") && j["unavailable_reason"].is_string()) ? j["unavailable_reason"].get<std::string>() : "";
...
int main(int argc, char* argv[]) {
    ...
    std::ifstream yf_f("json/yf_temp.json");
    if (yf_f.good()) {
        ...
        std::ifstream edgar_f("json/edgar_temp.json");
        if (edgar_f.is_open()) {
            try { unified["edgar"] = json::parse(edgar_f); } catch(...) {}
            edgar_f.close();
        }
        
        std::ifstream news_f("json/news_temp.json");
        if (news_f.is_open()) {
            try { unified["news"] = json::parse(news_f); } catch(...) {}
            news_f.close();
        }
        
        std::ifstream transcript_f("json/transcript_temp.json");
        if (transcript_f.is_open()) {
            try { unified["transcript"] = json::parse(transcript_f); } catch(...) {}
            transcript_f.close();
        }
    ...
    // Parse unified JSON
    std::vector<FinancialStatement> statements;
    TranscriptDocument t_doc;
    t_doc.available = false;
    std::map<std::string, EdgarDocument> filings;
    ...
    if (!unified_json_str.empty()) {
        try {
            ...
            if (unified.contains("edgar") && !unified["edgar"].is_null()) {
                filings = parse_edgar_json(unified["edgar"]);
            }
            if (unified.contains("transcript") && !unified["transcript"].is_null()) {
                t_doc = parse_transcript_json(unified["transcript"]);
            }
    ...
    if (!t_doc.available) {
        ...
        storage.clear_ticker_data(ticker, {"transcript_utterances"});
    ...
    if (filings.empty()) {
        ...
        storage.clear_ticker_data(ticker, {"filing_sections"});
    ...
    if (t_doc.available) {
        for (const auto& u : t_doc.utterances) {
            SentimentInput si;
            si.text = u.text;
            si.segment = "TRANSCRIPT_" + u.section;
            si.source_url = "Earnings Call Transcript"; // We can hardcode for transcripts
            si.title = "Earnings Call (" + u.speaker + ")";
            si.published_at = "Recent";
            si.ticker = ticker;
            batch_items.push_back(si);
        }
    }
    ...
    std::ifstream peers_f("json/peers_temp.json");
    if (peers_f.good()) {
        try {
    ...
    std::cout << "\n--- Generating Native Markdown Report ---" << std::endl;
    TemplateWriter::generate_markdown(ticker, db_path, project_root + "/reports");
