...
std::vector<RatioResult> RatioAgent::compute_ratios(
    ...
    if (!ebitda_item) {
        ebitda_item = get_line_item_full(income, "ebitda");
        fallback_used = true;
    }
    ...
    if (fallback_used && ev_val && ebitda_val) {
        auto mc_item = get_line_item_full(key_stats, "market_cap");
        auto pe_item = get_line_item_full(key_stats, "trailing_pe");
        auto ni_item = get_line_item_full(income, "net_income");
        
        if (mc_item && pe_item && ni_item && 
            mc_item->value.has_value() && pe_item->value.has_value() && ni_item->value.has_value() &&
            pe_item->value.value() != 0.0 && ni_item->value.value() != 0.0) {
            
            double implied_pe = mc_item->value.value() / ni_item->value.value();
            double scale_factor = implied_pe / pe_item->value.value();
            
            // If scale factor reveals a massive currency/magnitude mismatch (e.g. > 2.0 or < 0.5)
            if (scale_factor > 2.0 || scale_factor < 0.5) {
                ebitda_val = ebitda_val.value() * scale_factor;
            }
        }
    }
